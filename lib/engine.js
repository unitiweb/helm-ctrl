...
function expandCmdRefs(cmdStr, index, depth = 0) {
  if (depth > 10) throw new Error('Circular command reference detected')
  const spaceIdx = cmdStr.indexOf(' ')
  const firstWord = spaceIdx === -1 ? cmdStr : cmdStr.slice(0, spaceIdx)
  const rest = spaceIdx === -1 ? '' : cmdStr.slice(spaceIdx + 1)
  const ref = index.get(firstWord)
  if (!ref) return cmdStr
  const expanded = expandCmdRefs(ref.cmd, index, depth + 1)
  return rest ? `${expanded} ${rest}` : expanded
}

function run(def, extraArgs, helmVars, index) {
  const expandedCmd = index ? expandCmdRefs(def.cmd, index) : def.cmd
  const resolvedCmd = substituteVars(expandedCmd, helmVars)
...
async function main(config, argv) {
  ...
  if (!argv.length) {
    ...
    run(selection.command, selection.extraArgs, helmVars, index)
    return
  }
  ...
  run(command, extraArgs, helmVars, index)
}

module.exports = { main, loadDotEnv, parseEnvFile, resolveHelmVars, substituteVars, matches, resolveCommand, buildIndex, expandCmdRefs }
